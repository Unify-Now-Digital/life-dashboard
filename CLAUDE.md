...
**Adding a habit**: a dashed "+ Add habit" tile on the Habits screen (`HabitFooter.jsx`'s `AddHabitTile`) prompts for a name and a weekly frequency (1-7), always `period: 7` — there's no UI for a non-weekly cadence. `Dashboard.jsx`'s `addHabit()` builds the new habit object (`active: true`, `commitment: null`, `priority: false`) and seeds empty `habitLog`/`habitNoLog` arrays for it. The `key` is slugified from the label via `slugifyHabitKey()` (`src/lib/habits.js`), de-duped against every existing key — active *or* archived — so a retired habit's old log history can never be silently inherited by a new one sharing its name. No schema migration needed for this — it's runtime app state, not a new field on an existing persisted shape (contrast with `commitment`/`priority` themselves, which needed a migration because they were new fields added to *already-persisted* habit objects). New habits get the generic dot icon (`HabitIcon`'s default case) — only Spanish/Gym/Clean/Sleep have bespoke icons; there's no icon picker.

Every consumer of `state.habits` (`dailyFocus.js`, `habitStats.js`, `weeklyReview.js`, `assistantContext.js`, the assistant's `log_habit` tool) is already fully data-driven off the array — none of them hardcode a habit count or specific keys, so a new habit needs no code changes anywhere else to be ranked, logged (including by the assistant, since `log_habit`'s `habit_key` is a free string, not an enum — see `api/chat.js`), or reviewed. The one thing that *did* need a fix for a variable habit count: the desktop Habits-screen layout was a non-wrapping flex row, sized correctly for exactly 4 cards — it now wraps (`flexWrap: "wrap"`, each card `flex: "1 1 200px"`) so a 5th+ habit drops to a new row instead of every card getting squeezed.

There's no **remove/archive** UI yet — habits already carry an `active` flag that `dailyFocus.js` and the Habits screen both respect (`h.active !== false`), so archiving (vs. a hard delete, which would lose logged history) is the natural next step whenever it's wanted.
